...
const sketch: Sketch<P5Props> = (p5) => {
	...
	p5.draw = () => {
		...
		vectors = [
			...vectors.slice(-3000),
			...Array.from({ length: 15 }, () => p5.createVector(p5.random(-1, 1), p5.random(-1, 1), p5.random(-1, 1))),
		];

		vectors.forEach((v, i) => {
			const r = 4 + 9 / p5.sin(Math.sqrt(v.x * v.x + v.y * v.y + v.z * v.z) * 4 - time / 99);
			v.x += (p5.sin(v.y * r) / i) * 9;
			v.y += (p5.cos(r * v.x) / i) * 9;

			p5.stroke((i % 15) * 17, (i % 5) * 51, (i % 10) * 26);
			p5.point(v.x * (widthRatio * 109) + widthRatio * 270, v.y * (heightRatio * 109) + heightRatio * 270);
		});
	};
};
