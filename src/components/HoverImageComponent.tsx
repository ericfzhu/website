		...
		<span className={`${className} duration-300 pointer-events-auto`} onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave}>
			...
			{paths.map((path, i) => (
				<Image
					...
					height={200}
					width={300}
					className={`fixed z-[1] ${hover ? 'opacity-100' : 'opacity-0'} h-[35%] max-h-72 w-auto duration-300 transition-opacity -translate-y-1/2 -translate-x-1/2 pointer-events-none ${imageClassName} ${i === index ? 'visible' : 'invisible'}`}
					style={{
