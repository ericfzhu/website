...
export default function Library() {
	...
	for (let bookYear in booksByYear) {
		booksByYear[bookYear].sort((a, b) => new Date(b.date_finished || '').getTime() - new Date(a.date_finished || '').getTime());
	}
			...
			{Object.entries(booksByYear)
				...
				.map(([year, booksForYear]) => (
					<div className="mb-12 px-8" key={year}>
						<h2 className="text-4xl md:text-5xl lg:text-6xl mb-4 text-center pb-8 select-none ">{year}</h2>
